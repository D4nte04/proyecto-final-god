                ...
                <table class="table table-bordered table-tabla border-vanilla table-striped">
                    ...
                    <tbody>
                        <tr> 
                            ...
                            <td class="text-info">
                                <form method="POST"  action="{{route('profesor.comentarios',[$profesor->id,$propuesta->id])}}">
                                    @method('put')
                                    @csrf
                                    <textarea placeholder="Retroalimentación..." rows="4" cols="50" name="comentarios" class="form-control">{{$comentario->pivot->comentario}}</textarea>
                            </td>
                        </tr>
                    </tbody>
                </table>
                <div class="d-flex justify-content-end">
                                    <button class="btn btn-dark text-white fw-bold" type="submit" style="width: 210px;">Guardar Comentarios</button>
                                </form>
                                <form action="{{route('profesor.borrarComentarios',[$profesor->id,$propuesta->id])}}" method="POST">
                                    @method('delete')
                                    @csrf

                                    {{-- BORRAR COMENTARIO --}}
                                    <button class="btn btn-dark text-white fw-bold ms-3" type="submit" style="width: 210px;">Borrar Comentarios</button>
